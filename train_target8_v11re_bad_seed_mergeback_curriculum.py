...
def set_model_learning_rate(model: RecurrentPPO, learning_rate: float) -> None:
    ...
    if optimizer is None:
        raise RuntimeError("policy optimizer is missing; cannot override learning rate")
...
def main() -> None:
    ...
    parser.add_argument("--seed-base", type=int, default=10_841_000)
    parser.add_argument("--eval-seed-base", type=int, default=10_851_000)
    ...
    if tiny_mode:
        phase1_episodes = int(args.phase1_episodes or 64)
        phase2_episodes = int(args.phase2_episodes or 64)
        ...
        phase1_episodes = int(args.phase1_episodes or (125 if micro_mode else 250))
        phase2_episodes = int(args.phase2_episodes or (125 if micro_mode else 250))
        progress_interval = int(args.progress_interval or (125 if micro_mode else 250))
        phase1_checkpoint_interval = int(args.phase1_checkpoint_interval or (125 if micro_mode else 250))
        phase2_checkpoint_interval = int(args.phase2_checkpoint_interval or (125 if micro_mode else 250))
    ...
    csv_path = output_dir / "v11re_bad_seed_mergeback_results.csv"
    md_path = output_dir / "v11re_bad_seed_mergeback_results.md"
    ...
    try:
        env = SubprocVecEnv(
            [
                make_bad_seed_env(rank, STAGE.state_mode, seed_pool=seed_pool, env_overrides=phase1_overrides)
