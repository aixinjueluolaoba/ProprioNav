...
RUN_DIR=${RUN_DIR:-$ROOT/runs/目标8显式脱困v10阶段3方案A保留线_253}
WATCH_DIR=${WATCH_DIR:-$ROOT/runs/目标8显式脱困v10阶段3方案A保留线_checkpoint评估_253}
...
SEED_BASE=${SEED_BASE:-9815000}
EVAL_SEED_BASE=${EVAL_SEED_BASE:-9915000}
...
if test -n "$RESUME_MODEL" && test ! -f "$RESUME_MODEL"; then
  echo "resume_model_not_found path=$RESUME_MODEL" >&2
  exit 1
fi
if test -z "$RESUME_MODEL" && test ! -f "$INIT_POLICY_STATE_DICT"; then
  echo "resume_or_init_not_found resume=$RESUME_MODEL init=$INIT_POLICY_STATE_DICT" >&2
  exit 1
fi
