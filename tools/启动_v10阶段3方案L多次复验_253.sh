...
RUN_GROUP_NAME=${RUN_GROUP_NAME:-目标8显式脱困v10阶段3方案L多次复验_253}
...
ENV_OVERRIDES_FILE=${ENV_OVERRIDES_FILE:-$ROOT/configs/v10_stage3/方案L_K轻收角.json}
RUN_COUNT=${RUN_COUNT:-3}
SEED_BASE_START=${SEED_BASE_START:-13355000}
...
printf '%s\n' \
  'run_id,run_name,train_dir,watch_dir,seed_base,eval_seed_base,stage3_episodes,checkpoint_interval,env_overrides_file,init_policy_state_dict' \
  > "$MANIFEST_PATH"
...
for ((index = 1; index <= RUN_COUNT; index++)); do
  ...
  run_name="${run_id}_方案L_K轻收角_${run_label}"
