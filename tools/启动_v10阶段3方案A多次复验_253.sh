...
RUN_GROUP_NAME=${RUN_GROUP_NAME:-目标8显式脱困v10阶段3方案A多次复验_253}
...
ENV_OVERRIDES_FILE=${ENV_OVERRIDES_FILE:-$ROOT/configs/v10_stage3/方案A_收紧gate.json}
RUN_COUNT=${RUN_COUNT:-3}
SEED_BASE_START=${SEED_BASE_START:-16155000}
...
for ((index = 1; index <= RUN_COUNT; index++)); do
  ...
  run_name="${run_id}_方案A收紧gate_${run_label}"
