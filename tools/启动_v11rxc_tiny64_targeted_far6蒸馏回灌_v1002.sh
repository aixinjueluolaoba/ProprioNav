...
RUN_NAME=${RUN_NAME:-v11rxc_tiny64_targeted_far6_distill_tiny}
RUN_DIR=${RUN_DIR:-$REMOTE_ROOT/runs/$RUN_NAME}
DATA_DIR=$RUN_DIR/teacher_datasets
BASE_MODEL=${BASE_MODEL:-$REMOTE_ROOT/runs/v11rxc_nlite125_fix6300031_targeted_far6_stuck10_tiny64/rppo_medium_lstm128x2_observable12_target8_macro_library_v11rxc_stage2/rppo_medium_lstm128x2_observable12_target8_macro_library_v11rxc_stage2.zip}
...
BASE_OVERRIDES_REMOTE=${BASE_OVERRIDES_REMOTE:-$REMOTE_ROOT/configs/v11/方案N9_v11rxc近N6但wrapper不注入macro6评估探针.json}
SPECIALIST_OVERRIDES_REMOTE=${SPECIALIST_OVERRIDES_REMOTE:-$REMOTE_ROOT/configs/v11/方案Nlite_v11rxc修6300031远距定向macro6探针.json}
EVAL_OVERRIDES_REMOTE=${EVAL_OVERRIDES_REMOTE:-$BASE_OVERRIDES_REMOTE}
NORMAL_DATASET=$DATA_DIR/普通锚点轨迹_policy_tiny64_n9_v11rxc.npz
PRESSURE_DATASET=$DATA_DIR/压力锚点轨迹_policy_tiny64_n9_v11rxc.npz
SPECIALIST_DATASET=$DATA_DIR/坏种子targeted_far6_effective_tiny64_v11rxc.npz
...
PRESSURE_ACTION_LABEL_SOURCE=${PRESSURE_ACTION_LABEL_SOURCE:-policy}
SPECIALIST_ACTION_LABEL_SOURCE=${SPECIALIST_ACTION_LABEL_SOURCE:-effective_macro}
DATASET_WEIGHTS=${DATASET_WEIGHTS:-1.0,1.2,2.4}
DATASET_ROLES=${DATASET_ROLES:-base,base,specialist}
DATASET_LOSS_HEADS=${DATASET_LOSS_HEADS:-all,all,macro_only}
TRAINABLE_PARAM_MODE=${TRAINABLE_PARAM_MODE:-macro_slice_only}
EPOCHS=${EPOCHS:-8}
...
NORMAL_EXPORT_CMD="
  ...
  python export_v11_teacher_dataset.py \
    ...
    --device cpu \
    --env-overrides-file '$BASE_OVERRIDES_REMOTE'
"
