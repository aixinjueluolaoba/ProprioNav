...
RUN_GROUP_NAME=${RUN_GROUP_NAME:-目标8显式脱困v10阶段3平衡微调对照_253}
...
VARIANT_NAMES=${VARIANT_NAMES:-方案F保守平衡|方案G轻压平衡|方案H激进平衡}
ENV_OVERRIDES_FILES=${ENV_OVERRIDES_FILES:-$ROOT/configs/v10_stage3/方案F_保守平衡.json|$ROOT/configs/v10_stage3/方案G_轻压平衡.json|$ROOT/configs/v10_stage3/方案H_激进平衡.json}
SEED_BASE_START=${SEED_BASE_START:-9855000}
SEED_STRIDE=${SEED_STRIDE:-200000}
EVAL_SEED_OFFSET=${EVAL_SEED_OFFSET:-100000}
STAGE3_EPISODES=${STAGE3_EPISODES:-500}
...
if test "${#variant_names[@]}" -ne "$RUN_COUNT"; then
  echo "variant_name_count_mismatch expected=$RUN_COUNT actual=${#variant_names[@]}" >&2
  exit 1
fi
if test "${#env_override_files[@]}" -ne "$RUN_COUNT"; then
