...
TRAIN_DIR=/home/diana/fishing/blind_nav_rl/runs/目标8显式脱困v10阶段3短训选优_v1002
WATCH_DIR=/home/diana/fishing/blind_nav_rl/runs/目标8显式脱困v10阶段3短训选优_checkpoint评估_v1002
FINAL_DIR=/home/diana/fishing/blind_nav_rl/runs/目标8显式脱困v10阶段3短训选优_最终归档_v1002
...
if test -f "$TRAIN_DIR/训练.pid" && ps -p "$(cat "$TRAIN_DIR/训练.pid")" >/dev/null 2>&1; then
  ...
  TRAIN_CMD="
    ...
    VIDEO_EPISODES=0 \
    SEED_BASE=9815000 \
    EVAL_SEED_BASE=9915000 \
    CHECKPOINT_INTERVAL=500 \
    PROGRESS_INTERVAL=250 \
    bash launch_target8_v10_stage2_resume.sh
  "
  setsid bash -lc "$TRAIN_CMD" > "$TRAIN_DIR/阶段3短训.log" 2>&1 &
