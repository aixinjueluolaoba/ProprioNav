...
RUN_NAME=${RUN_NAME:-v11rxc_n19_pressure_hybrid_anchor_tiny}
...
PRESSURE_SPECIALIST_MODEL=${PRESSURE_SPECIALIST_MODEL:-$RESUME_MODEL}
OVERRIDES_REMOTE=${OVERRIDES_REMOTE:-$REMOTE_ROOT/configs/v11/方案N19_v11rxc中距抑制宏长链覆写评估探针.json}
NORMAL_DATASET=$DATA_DIR/普通锚点轨迹_v11rxc_N19.npz
PRESSURE_SPECIALIST_DATASET=$DATA_DIR/压力专科轨迹_v11rxc_N19.npz
NORMAL_SEED_START=${NORMAL_SEED_START:-1810000}
NORMAL_EPISODES=${NORMAL_EPISODES:-24}
PRESSURE_SPECIALIST_SEEDS=${PRESSURE_SPECIALIST_SEEDS:-5200015,5200015,5200008,5200022}
TRAIN_EXTRA_ARGS=${TRAIN_EXTRA_ARGS:---tiny}
ANCHOR_DATASET_ROLES=${ANCHOR_DATASET_ROLES:-base,specialist}
ANCHOR_DATASET_WEIGHTS=${ANCHOR_DATASET_WEIGHTS:-1.0,1.8}
RECOVERY_WEIGHT=${RECOVERY_WEIGHT:-1.3}
...
ANCHOR_TRAINABLE_PARAM_MODE=${ANCHOR_TRAINABLE_PARAM_MODE:-all}
SEED_POOL=${SEED_POOL:-5200015,5200015,5200008,5200022}
...
copy_file train_target8_v11rxc_pressure_hybrid_anchor_finetune.py
copy_file "configs/v11/方案N19_v11rxc中距抑制宏长链覆写评估探针.json"

ssh v1002 "mkdir -p '$RUN_DIR' '$DATA_DIR'"

for required in "$RESUME_MODEL" "$NORMAL_MODEL" "$PRESSURE_SPECIALIST_MODEL"; do
...
PRESSURE_SPECIALIST_EXPORT_CMD="
  cd $REMOTE_ROOT &&
  source ~/miniconda3/etc/profile.d/conda.sh &&
  conda activate ML &&
  python export_v11_teacher_dataset.py \
    --model '$PRESSURE_SPECIALIST_MODEL' \
    --out '$PRESSURE_SPECIALIST_DATASET' \
    --scenario pressure \
    --seed-list '$PRESSURE_SPECIALIST_SEEDS' \
    --state-mode observable12_target8_macro_library_v11rxc_stage2 \
    --device cpu \
    --env-overrides-file '$OVERRIDES_REMOTE'
"
maybe_export_dataset "$PRESSURE_SPECIALIST_DATASET" "$PRESSURE_SPECIALIST_EXPORT_CMD"

ssh v1002 "cd '$REMOTE_ROOT' && source ~/miniconda3/etc/profile.d/conda.sh && conda activate ML && python train_target8_v11rxc_pressure_hybrid_anchor_finetune.py \
  $TRAIN_EXTRA_ARGS \
  --output-dir '$RUN_DIR' \
  --resume-model '$RESUME_MODEL' \
  --anchor-datasets '$NORMAL_DATASET' '$PRESSURE_SPECIALIST_DATASET' \
