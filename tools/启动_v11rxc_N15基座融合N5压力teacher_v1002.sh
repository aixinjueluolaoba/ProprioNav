...
RUN_NAME=${RUN_NAME:-v11rxc_n15_anchor_n5_pressure5200015_tiny}
...
TRAIN_OVERRIDES_REMOTE=${TRAIN_OVERRIDES_REMOTE:-$REMOTE_ROOT/configs/v11/方案N15_v11rxc中距generic限宏型评估探针.json}
...
SPECIALIST_OVERRIDES_REMOTE=${SPECIALIST_OVERRIDES_REMOTE:-$REMOTE_ROOT/configs/v11/方案N5_v11rxc策略宏需释放评估探针.json}
NORMAL_DATASET=$DATA_DIR/普通锚点轨迹_N15_v11rxc.npz
PRESSURE_DATASET=$DATA_DIR/压力锚点轨迹_N15_v11rxc.npz
SPECIALIST_DATASET=$DATA_DIR/压力特化轨迹_N5_v11rxc.npz
...
SPECIALIST_SEED_POOL=${SPECIALIST_SEED_POOL:-5200015,5200015,5200022}
TRAIN_EXTRA_ARGS=${TRAIN_EXTRA_ARGS:---tiny}
ANCHOR_DATASET_ROLES=${ANCHOR_DATASET_ROLES:-base,base,specialist}
ANCHOR_DATASET_WEIGHTS=${ANCHOR_DATASET_WEIGHTS:-1.1,1.3,2.0}
...
copy_file "configs/v11/方案N15_v11rxc中距generic限宏型评估探针.json"
copy_file "configs/v11/方案N5_v11rxc策略宏需释放评估探针.json"
...
echo "train_overrides=$TRAIN_OVERRIDES_REMOTE"
echo "normal_overrides=$NORMAL_OVERRIDES_REMOTE"
echo "pressure_overrides=$PRESSURE_OVERRIDES_REMOTE"
echo "specialist_overrides=$SPECIALIST_OVERRIDES_REMOTE"
echo "seed_pool=$SEED_POOL"
echo "online_bad_seed_pool=$ONLINE_BAD_SEED_POOL"
echo "pressure_seed_pool=${PRESSURE_SEED_POOL:-scan}"
...
# Specialist dataset reuses the same policy family but swaps to the N5 wrapper surface
# on explicit pressure seeds so BC only sees the better 5200015-style recovery line.
