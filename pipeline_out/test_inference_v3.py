"""Compare V3 TorchScript math against the Rust/NCNN low-level FFI."""
...
from export_v3_ncnn import RecurrentInferenceV3
...
def main():
    from run_pipeline import RecurrentActorCritic

    state = torch.load(BASE / "policy_weights_v3_hybrid_sharp.pth", map_location="cpu")
    model = RecurrentActorCritic(state_dim=10, hidden_dim=96, actor_width=48, macro=False)
    model.load_state_dict(state)
    reference = RecurrentInferenceV3(model).eval()
    ...
    handle = library.init_net(
        str(BASE / "policy.param").encode(),
        str(BASE / "policy.bin").encode(),
    )
    if not handle:
