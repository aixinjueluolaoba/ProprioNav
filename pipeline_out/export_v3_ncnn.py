"""Export the V3 hybrid navigation policy for PNNX/NCNN deployment."""
...
class RecurrentInferenceV3(nn.Module):
    """Single-step LSTM inference; jump is handled by the SO probe controller."""
...
def main():
    ...
    parser.add_argument(
        "--weights",
        default=str(BASE / "pipeline_out" / "policy_weights_v3_hybrid_sharp.pth"),
    )
    parser.add_argument(
        "--output",
        default=str(BASE / "pipeline_out" / "policy_v3.pt"),
    )
    ...
    if hidden_dim != 96:
        raise ValueError(f"V3 exporter expects hidden_dim=96, got {hidden_dim}")

    model = RecurrentActorCritic(
        state_dim=10, hidden_dim=hidden_dim, actor_width=actor_width, macro=False
    )
    model.load_state_dict(state)
    model.eval()
    inference = RecurrentInferenceV3(model).eval()
