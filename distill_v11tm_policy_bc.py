...
def map_v11re_actions_to_v11tm(actions: np.ndarray) -> np.ndarray:
    ...
    mapped = np.zeros((actions_np.shape[0], 4), dtype=np.int64)
    mapped[:, 0] = np.clip(actions_np[:, 0], 0, 4)
    mapped[:, 1] = np.clip(actions_np[:, 1], 0, 1)
    macro_bin = np.clip(actions_np[:, 2], 0, 6)
    mapped[:, 2] = (macro_bin != 0).astype(np.int64)
    mapped[:, 3] = np.where(macro_bin > 0, macro_bin - 1, 0)
    return mapped
...
def main() -> None:
    ...
    for dataset_path, dataset_weight, dataset_role in zip(dataset_paths, dataset_weights, dataset_roles, strict=True):
        ...
        actions_parts.append(map_v11re_actions_to_v11tm(np.asarray(data["actions"], dtype=np.int64)[mask]))
    ...
    for epoch in range(int(args.epochs)):
        ...
        del values
        loss = -(log_prob * sample_weights).sum() / sample_weights.sum().clamp_min(1e-6) - 1e-3 * entropy.mean()
        loss.backward()
