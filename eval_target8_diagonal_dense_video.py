...
def evaluate_one(
    ...
    row = {
        ...
        "tree_count": env.tree_count,
        "mountain_count": env.mountain_count,
...
def summarize(rows: list[dict[str, Any]], video_path: Path | None) -> dict[str, Any]:
...
def main() -> None:
    ...
    parser.add_argument("--seed-start", type=int, default=6_300_000)
    ...
    for index in range(args.episodes):
        ...
        row, trace = evaluate_one(model, seed, args.state_mode, env_overrides=env_overrides)
        rows.append(row)
        print(
            "diagonal_eval index={index} seed={seed} success={success} steps={steps} "
        ...
        if index < args.video_episodes:
            video = out_dir / f"diagonal_dense_eval_{index + 1:02d}.mp4"
            render_episode(trace, video, fps=args.fps)
            videos.append(video)
    concat_path: Path | None = None
    if videos:
        concat_path = out_dir / "diagonal_dense_eval_concat.mp4"
        concat_videos(videos, concat_path)
    write_csv(out_dir / "diagonal_dense_eval_results.csv", rows)
    write_csv(out_dir / "diagonal_dense_eval_summary.csv", [summarize(rows, concat_path)])
