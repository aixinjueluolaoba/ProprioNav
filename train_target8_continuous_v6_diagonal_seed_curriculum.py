...
def main() -> None:
    ...
    seed_pool = parse_seed_pool(args.seed_pool) if args.seed_pool else list(DEFAULT_DIAGONAL_SEEDS)
    ...
    (output_dir / "diagonal_seed_pool.txt").write_text("\n".join(str(seed) for seed in seed_pool) + "\n", encoding="utf-8")
    csv_path = output_dir / "continuous_v6_diagonal_seed_results.csv"
    md_path = output_dir / "continuous_v6_diagonal_seed_results.md"
    ...
    try:
        ...
        model = make_model(STAGE, env, int(seed_pool[0]), args.device)
