...
def configure_anchor_trainable_params(model: RecurrentPPO, mode: str) -> tuple[str, tuple[int, int] | None, list[torch.nn.Parameter], int]:
    ...
    if normalized == "all":
        ...
        raise ValueError(f"unsupported anchor trainable param mode: {mode}")
    trainable_tensors = [param for param in model.policy.parameters() if param.requires_grad]
    trainable = sum(param.numel() for param in trainable_tensors)
    return normalized, macro_slice, trainable_tensors, int(trainable)


def apply_anchor_gradient_mask(
    model: RecurrentPPO,
...
def load_anchor_dataset(
    ...
    success_only: bool,
    stuck_no_progress_threshold: float,
    ...
    for dataset_path, dataset_weight, dataset_role in zip(dataset_paths, dataset_weights, dataset_roles, strict=True):
        ...
        if success_only and "teacher_success" in data:
        ...
        stuck_like = (no_progress_raw >= stuck_no_progress_threshold) | (stuck_time_raw >= stuck_time_threshold)
        recovery_like = recovery_mode_raw != 0
        if dataset_role == "base":
            mask &= ~(stuck_like | recovery_like | collided_raw)
        elif dataset_role == "specialist":
    ...
    return {
        "obs": obs_np,
        "actions": actions_np,
        "episode_starts": episode_starts_np,
        "no_progress_time": no_progress_np,
        "stuck_time": stuck_time_np,
        "collided": collided_np,
        "recovery_mode": recovery_mode_np,
        "dataset_weight": dataset_weight_np,
    }
...
def compute_anchor_weights(
    ...
    weights = np.ones(int(dataset["obs"].shape[0]), dtype=np.float32)
    weights *= dataset["dataset_weight"]
    recovery_active = dataset["recovery_mode"] != 0
    stuck_active = (dataset["no_progress_time"] >= stuck_no_progress_threshold) | (
        dataset["stuck_time"] >= stuck_time_threshold
    )
    weights[recovery_active] *= float(recovery_weight)
    weights[stuck_active] *= float(stuck_weight)
    weights[dataset["collided"]] *= float(collision_weight)
    return weights
...
def make_mixed_env(
    ...
    rank: int,
    state_mode: str,
    online_bad_seed_pool: list[int],
    bad_seed_env_count: int,
    env_overrides: dict[str, dict[str, object]] | None = None,
):
    if rank < bad_seed_env_count:
        return make_bad_seed_env(rank, state_mode, seed_pool=online_bad_seed_pool, env_overrides=env_overrides)
...
def main() -> None:
    ...
    parser.add_argument("--seed-base", type=int, default=11_061_000)
    parser.add_argument("--eval-seed-base", type=int, default=11_071_000)
    ...
    parser.add_argument("--online-trainable-param-mode", default="all")
    ...
    parser.add_argument("--online-bad-seed-pool", default=None)
    ...
    online_bad_seed_pool = parse_seed_pool(args.online_bad_seed_pool) if args.online_bad_seed_pool else list(seed_pool)
    ...
    (output_dir / "online_bad_seed_pool.txt").write_text(
        "\n".join(str(seed) for seed in online_bad_seed_pool) + "\n",
        encoding="utf-8",
    )
    csv_path = output_dir / "v11rxc_hybrid_anchor_results.csv"
    md_path = output_dir / "v11rxc_hybrid_anchor_results.md"
    ...
    try:
        bad_seed_env_count = int(round(int(args.n_envs) * phase2_bad_seed_ratio))
        env = SubprocVecEnv(
            [
                make_mixed_env(
                    int(args.seed_base),
                    rank,
                    STAGE.state_mode,
                    online_bad_seed_pool=online_bad_seed_pool,
        ...
        online_trainable_mode, online_macro_slice, _online_trainable_tensors, online_trainable = configure_anchor_trainable_params(
            model, args.online_trainable_param_mode
        )
        print(
            f"online_trainable_param_mode={online_trainable_mode} online_trainable_params={online_trainable} macro_slice={online_macro_slice}",
            flush=True,
        )
        ...
        for round_index in range(int(hybrid_rounds)):
            configure_anchor_trainable_params(model, online_trainable_mode)
            rebuild_policy_optimizer(model, float(args.ppo_learning_rate))
            original_online_step = model.policy.optimizer.step

            def masked_online_step(closure=None):
                apply_anchor_gradient_mask(model, online_trainable_mode, online_macro_slice)
                if closure is None:
                    return original_online_step()
                return original_online_step(closure)

            model.policy.optimizer.step = masked_online_step  # type: ignore[method-assign]
            ...
            configure_anchor_trainable_params(model, anchor_trainable_mode)
        ...
        restore_full_policy_optimizer_for_save(model, float(args.ppo_learning_rate))
        ...
        row: dict[str, object] = {
            ...
            "online_bad_seed_pool_size": len(online_bad_seed_pool),
            "online_bad_seed_pool_path": str(output_dir / "online_bad_seed_pool.txt"),
            "phase2_bad_seed_ratio": phase2_bad_seed_ratio,
            "phase2_bad_seed_env_count": bad_seed_env_count,
            "ppo_learning_rate": float(args.ppo_learning_rate),
            "anchor_learning_rate": float(args.anchor_learning_rate),
            "online_trainable_param_mode": online_trainable_mode,
            "online_trainable_params": online_trainable,
